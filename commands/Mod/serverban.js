...
module.exports = {
  ...
  example: ['guildban @Purple posting ads', 'guildban 499250383785558026 posting ads'],
  args: true,
  guildOnly: true,
  adminonly: true,
	...
	async execute(message, args, client) {
        ...
        if (/^y(?:e(?:a|s)?)?$/i.test(response.content)) {
            ...
            sentMessage = await message.channel.send(`Banning **${member}**...`);
        ...
        try {
          ...
          sentMessage.edit(`Successfully banned **${member}**...`);
          if (tag) {
            tag.increment('bans');
          }
    
        } catch (error) {
          sentMessage.edit(`I could not ban **${member}** <:notlikecat:529505687773118484>`);
        }
	},
};
