...
class TagStatsCommand extends Command {
    ...
    async exec(message, { member }) {
        ...
        if (member) {
            const top = await Tags.findOne({ where: { uses: { [Op.lt] : 10000 }, guildID: message.guild.id, authorID: member.user.id }});
            ...
            .addField('Top Tag', `${top.name} ( ${top.uses} uses )`)
            .setFooter(message.guild.name, message.guild.iconURL()).setColor(0x8387db)
            return message.util.send({ embed });
        }
        const top = await Tags.findOne({ where: { uses: { [Op.lt] : 10000 }, guildID: message.guild.id }});
        ...
        .addField(`Total Tags`, tags.length).addField(`Total Uses`, totaluses).addField('Top Tag', `${top.name} ( ${top.uses} uses)`).setColor(0x8387db)
        return message.util.send({ embed });
    }
}
