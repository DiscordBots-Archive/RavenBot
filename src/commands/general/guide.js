...
class GuideCommand extends Command {
	...
	async exec(message) {
		...
		const embed = this.client.util.embed()
			...
			.setTitle('Guide')
			.addField('Setup', [
				`${this.client.user.username.replace(/(\b\w)/gi, lc => lc.toUpperCase())} requires the following permissions to be usable:`,
				'- `Read Messages`',
				'- `Manage Messages`',
			...
			.addField('Other', [
				...
				'',
				`For more information about ${this.client.user.username.replace(/(\b\w)/gi, lc => lc.toUpperCase())}, check out \`${prefix}about\` and \`${prefix}stats\`.`,
				`Invite ${this.client.user.username.replace(/(\b\w)/gi, lc => lc.toUpperCase())} to your server with \`${prefix}invite\`.`
			]);
